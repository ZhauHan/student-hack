        ...
        props.planets.forEach((otherPlanet) => {
          if (otherPlanet.planetName !== currPlanet.planetName) {
            ...
            const direction = new THREE.Vector3()
              .subVectors(otherPlanet.position, currPlanet.position)
              .multiplyScalar(1000000);
            const distanceSquared = direction.lengthSq();
            const forceMagnitude =
              (G * currPlanet.mass * otherPlanet.mass) / distanceSquared;
            const force = direction.normalize().multiplyScalar(forceMagnitude);

            acceleration.add(
              force.divideScalar(currPlanet.mass).multiplyScalar(2)
            );
            const distance = currPlanet.position.distanceTo(
  ...
  return (
    <>
      {props.planets.map((planetData: PlanetData, index: number) => (
        <Planet key={index} planetData={planetData} />
      ))}
    </>
  );
}
